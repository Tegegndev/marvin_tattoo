...
async function main() {
  ...
  if (servicesCount === 0) {
    ...
    console.log(`✅ Seeded ${services.length} studio service disciplines.`);
  }

  // 4. Seed Portfolio Pieces
  const portfolioCount = await prisma.portfolioPiece.count();
  if (portfolioCount === 0) {
    const pieces = [
      {
        ...
        imageUrl: "/images/portfolio/portrait-elder-woman.png",
        description:
          "A heartfelt tribute to motherhood — high-detail black-and-grey photo-realism with soft skin transitions and intricate patterned headwrap texturing.",
    ...
    console.log(`✅ Seeded ${pieces.length} portfolio pieces.`);
  }
  ...
  if (productsCount === 0) {
    const products = [
      {
        name: "Wireless Rotary Pen Machine",
        category: "Hard Goods",
      ...
      {
        name: "Sterile Cartridge Needles (Box of 20)",
        category: "Needles",
    ...
    console.log(`✅ Seeded ${products.length} shop items.`);
  }
  ...
  if (testimonialsCount === 0) {
    ...
    console.log(`✅ Seeded ${reviews.length} client testimonials.`);
  }
